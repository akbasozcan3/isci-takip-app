...
export default function VerifyPhone(): React.JSX.Element {
  ...
  const verify = async () => {
    if (!phone || !code) {
      message.show({ type: 'error', title: 'Eksik Bilgi', description: 'Telefon ve kod zorunludur.' });
      return;
    }
    try {
      setLoading(true);
      const res = await fetch(`${getApiBase()}/auth/verify-phone`, {
        method: 'POST',
      ...
      message.show({ type: 'success', title: 'Telefon Doğrulandı', description: 'Artık giriş yapabilirsiniz.' });
  ...
  const resend = async () => {
    ...
    try {
      setResending(true);
      const res = await fetch(`${getApiBase()}/auth/send-phone-code`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <Animated.View style={{ flex: 1, backgroundColor: '#0f172a', padding: 24, justifyContent: 'center', opacity: fade, transform: [{ translateY: translate }] }}>
      ...
      <Text style={{ color: 'white', fontSize: 28, fontWeight: '700', marginBottom: 24 }}>Telefon Doğrulama</Text>
      ...
      <Button title="Telefonu Doğrula" onPress={verify} loading={loading} />
