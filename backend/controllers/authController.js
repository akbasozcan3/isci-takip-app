...
class AuthController {
  ...
  async preVerifyEmail(req, res) {
    try {
      ...
      try {
        ...
        console.log(`[Email] Attempting to send verification code to ${email} via ${emailServiceUrl}`);
        ...
        if (emailResponse.ok) {
          ...
          if (responseData.success) {
            ...
            emailError = responseData.error || 'Email servisi hata döndü';
            console.error(`❌ Email service returned error: ${emailError}`);
          }
        } else {
          const errorData = await emailResponse.json().catch(() => ({}));
          emailError = errorData.error || `HTTP ${emailResponse.status}`;
          console.error(`❌ Email service failed with status ${emailResponse.status}: ${emailError}`);
        }
      } catch (error) {
        if (error.name === 'AbortError') {
          emailError = 'Email servisi zaman aşımına uğradı';
          console.error('❌ Email service timeout after 15 seconds');
        } else if (error.message?.includes('ECONNREFUSED') || error.message?.includes('fetch failed')) {
          emailError = 'Email servisine bağlanılamadı. Servis çalışıyor mu kontrol edin.';
          console.error(`❌ Email service connection failed. Is email-service running on port 5001?`);
          console.error(`   Check: pm2 status`);
          console.error(`   Restart: pm2 restart email-service`);
        } else {
          emailError = `Email servisi hatası: ${error.message}`;
    ...
    try {
      ...
      (async () => {
        ...
        try {
          ...
          console.log(`[Email] Attempting to send reset link to ${email} via ${emailServiceUrl}`);
          
          const resetResponse = await fetch(`${emailServiceUrl}/send-reset-link`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
          ...
          if (resetResponse.ok) {
            const responseData = await resetResponse.json().catch(() => ({}));
            if (responseData.success) {
          sent = true;
              console.log(`✅ Reset link sent successfully via email service to ${email}`);
            } else {
              console.error(`❌ Email service returned error: ${responseData.error || 'Unknown error'}`);
            }
          } else {
            const errorData = await resetResponse.json().catch(() => ({}));
            console.error(`❌ Email service failed with status ${resetResponse.status}: ${errorData.error || 'Unknown error'}`);
          }
        } catch (e) {
          if (e.name === 'AbortError') {
            ...
            console.error(`❌ External email service failed: ${e?.message || e}`);
          }
        }
      })().catch(() => {});
  ...
  async register(req, res) {
    ...
    try {
      ...
      const sanitized = validationService.sanitizeUserInput({ email, password, displayName, verificationCode });
      
      const validation = validationService.validateUserData({
      ...
      if (!validation.valid) {
        return res.status(400).json({ 
          ...
          requiresVerification: !sanitized.verificationCode
        });
      }
