...
export const Button: React.FC<Props> = ({ title, onPress, loading, disabled, style, variant = 'primary' }) => {
  ...
  const textColor = '#fff';
  return (
    <Pressable onPress={onPress} disabled={disabled || loading} style={({ pressed }) => [styles.pressable, (disabled || loading) && styles.disabled, pressed && { opacity: 0.95 }, style]}>
